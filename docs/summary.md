...
The login modal inputs are protected from SQL injection attacks by using RegEx to limit allowed characters. Note that proper implementation with a database would be needed before it is actually vulnerable to said attack type, and that RegEx limiting is not a comprehensive defense.
